...
_HEADER = ["Score", "Company", "Job Title", "Location", "Country", "Source", "Job URL", "Date Found", "Status"]
...
class GoogleSheetExporter:
    ...
    def _get_worksheet(self) -> gspread.Worksheet:
        ...
        try:
            ...
            raise GoogleSheetError(f"Could not open Google Sheet {self._sheet_id!r}: {exc}") from exc
        ...
        try:
            ...
            try:
                ...
                raise GoogleSheetError(
                    f"Could not create worksheet {self._worksheet_name!r}: {exc}"
                ) from exc
        except Exception as exc:
            raise GoogleSheetError(
                f"Could not open worksheet {self._worksheet_name!r}: {exc}"
            ) from exc
        else:
            try:
                ...
                raise GoogleSheetError(f"Could not read/write header row: {exc}") from exc
    ...
    def export_leads(self, leads: Sequence[sqlite3.Row]) -> list[str]:
        ...
        try:
            ...
            raise GoogleSheetError(f"Failed to append {len(rows)} lead(s) to Google Sheet: {exc}") from exc
    ...
    def _lead_to_row(lead: sqlite3.Row) -> list[object]:
        ...
        order: Score, Company, Job Title, Location, Country, Source,
        Job URL, Date Found, Status."""
